...
- **`jovida due`** — the "what needs attention" radar: overdue todos plus those whose deadline **or reminder** falls within a window (default 24h; `--within 2h`/`90m`/`1d`). Read-only and served from a short-lived local cache, so it's cheap to call often — prefer it over assembling the same answer from `list` when the user asks "anything urgent / due soon?". `--brief` prints a one-liner (empty when nothing is due, silent on errors) built for statuslines and prompt hooks.
...
- **`jovida import lark`** — one-way, idempotent import of the user's incomplete Lark/Feishu tasks (needs `lark-cli` installed and signed in: `lark-cli auth login --domain task`). Safe to re-run: never duplicates, syncs Lark-side completions into Jovida, never writes back to Lark. Use `--dry-run` first when the user just wants to see what would come over.

// 重复待办的本地展开 + fork(对齐官方 app jovida-todo 的 todo_recurrence.dart / home_todo_recurrence.dart,
// 而非后端 —— 后端 get_todo_snapshot 只回原始规则,不展开「发生」;展开在客户端做)。
// 纯计算:不触网、不读状态。时间一律 Unix 秒;civil date 用本地时区。
import type { TodoEntry, TodoRecurring } from './types'
import { belongDateToSec } from './convert'
import { newSubtaskId } from './ids'

/** 本地历法日期(年/月 1-12/日 1-31)。 */
...
function seriesStart(s: TodoRecurring): YMD {
 ...
 */
export function seriesOccursOn(s: TodoRecurring, day: YMD): boolean {
  const start = seriesStart(s)
  if (cmp(day, start) < 0) return false
  const r = s.repeat
  if (r.endAt > 0 && cmp(day, ymdFromSec(r.endAt)) > 0) return false
  const interval = r.interval > 0 ? r.interval : 1
  switch (r.unit) {
    case 'day':
      return (dayOrdinal(day) - dayOrdinal(start)) % interval === 0
    case 'week': {
      if ((weekOrdinal(day) - weekOrdinal(start)) % interval !== 0) return false
      const wds = r.weekdays.filter((w) => w >= 1 && w <= 7)
      if (wds.length === 0) return isoWeekday(day) === isoWeekday(start) // 无 byWeekdays → 跟种子同一周几
      return wds.includes(isoWeekday(day))
    }
    case 'month': {
      if (((day.y - start.y) * 12 + (day.m - start.m)) % interval !== 0) return false
...
export function seriesOccurrencesInRange(
  ...
  if (cur < startDate) cur.setFullYear(start.y, start.m - 1, start.d)
  const end = new Date(toSec * 1000)
  end.setHours(0, 0, 0, 0)
  while (cur <= end && out.length < limit) {
    const c: YMD = { y: cur.getFullYear(), m: cur.getMonth() + 1, d: cur.getDate() }
    if (seriesOccursOn(s, c)) out.push(c)
...
export function occurrenceKey(recurringId: string, day: YMD): string {
...
/** 解析发生 id;非该形态返回 null。 */
export function parseOccurrenceId(id: string): ParsedOccurrence | null {
  if (!id.startsWith('recurring:')) return null
  const last = id.lastIndexOf(':')
  if (last <= 9) return null // 9 = 'recurring:' 里那个冒号的下标;须有第二个冒号
  const recurringId = id.slice(10, last)
  const secsStr = id.slice(last + 1)
  if (!recurringId || !/^\d+$/.test(secsStr)) return null
 ...
 */
export function occurrenceToEntry(s: TodoRecurring, day: YMD): TodoEntry {
  const occSec = belongDateToSec(ymdToKey(day)) // 本地当天 0 点秒
  let dueAt = 0
  if (s.dueAt > 0) {
    // series 带时刻 → 该发生继承「同一时分秒」落在发生日。
    const t = new Date(s.dueAt * 1000)
    dueAt = Math.floor(
      new Date(day.y, day.m - 1, day.d, t.getHours(), t.getMinutes(), t.getSeconds()).getTime() / 1000
    )
  }
  return {
    ...
    occurrenceAt: occSec,
    subtasks: s.subtasks.map((st) => ({ id: newSubtaskId(), title: st.title, completedAt: 0 })),
 ...
 */
export function expandRecurring(opts: {
  recurrings: TodoRecurring[]
  realEntries: TodoEntry[]
  scope: string
  status: string
  todayStart: number
  ...
  rangeTo: number
  horizonDays: number
  collapseHorizonDays: number
}): TodoEntry[] {
  const { recurrings, realEntries, scope, status, todayStart, tomorrowStart } = opts
  if (status === 'completed' || scope === 'recent') return []

  const forked = new Map<string, TodoEntry>()
  ...
  const out: TodoEntry[] = []
  const emitFull = (from: number, to: number): void => {
    for (const s of recurrings) {
      for (const day of seriesOccurrencesInRange(s, from, to)) {
        if (!forked.has(occurrenceKey(s.recurringId, day))) out.push(occurrenceToEntry(s, day))
      }
    }
  }
  ...
  if (scope === 'today') {
    ...
    for (const s of recurrings) {
      ...
      if (next) out.push(occurrenceToEntry(s, next))
    }
  }
  return out
}
